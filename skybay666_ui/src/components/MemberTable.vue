        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMembers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMembers">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Member</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MemberId" type="text" placeholder="Enter MemberId" v-model="memberToAdd.memberId"></base-input>
  <base-input label="Points" type="text" placeholder="Enter Points" v-model="memberToAdd.points"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="members" :row-key="record => record.MemberId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MemberPictureView :members="members" />
          </div>
...
import MemberPictureView from './MemberPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'memberId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMembersTable() {
      ...
      for (let i = 0; i < this.members.length; i++) {
        membersTableData.push({
          id: i,
          memberId: this.members[i].memberId,
          year: this.members[i].year,
          date: this.members[i].date,
          competitionId: this.members[i].competitionId,
          memberId: this.members[i].memberId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-members',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    routingToMemberDetail(id) {
      this.$router.push({ name: 'MemberDetail', params: { memberId: id.toString() }})
    },
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMembersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.memberToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.memberToAdd);
      console.log(jsonData);
      
      const res = await MemberService.addMember(jsonData);
